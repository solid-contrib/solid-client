 ...
 *   more access modes, each as either a lowercase string (for example, 'write'),
 *   or an RDF statement.
 */
 ...
 * @private
 * @param accessMode {String|Statement} Access mode as either a lowercase string
 *   (for example, 'write'), or an RDF statement.
 */
function addModeSingle (accessMode) {
  if (typeof accessMode !== 'string') {
    accessMode = accessModeFromRDF
  }
...
function removeMode (accessMode) {
  ...
  if (Array.isArray(accessMode)) {
    accessMode.forEach(function (ea) {
      delete self.accessModes[ea]
    })
  } else if (typeof accessMode === 'string') {
    delete self.accessModes[accessMode]
  } else {
    throw new RangeError('addMode() expects a string or an Array of strings')
  }
  return self
}
