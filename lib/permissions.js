...
var Permissions = {
   ...
   * @param permissions {[PermissionType]} List of permissions
  ...
  * @param agent {String} The WebID of the agent to retrieve the permissions for. Retrieve for all if undefined
  * @return {Promise({PermissionType:[String]})} Object holding each permission type and corresponding agents
  */
  getPermissions: function getPermissions (url, agent) {
    return new Promise(function (resolve, reject) {
...
* @method encodeModes
* @param modes {[String]} list of PermissionTypes
* @return {[String]}
*/
  ...
  return new Promise(function (resolve, reject) {
    Permissions.getPermissions(url).then(
      ...
      for (var i in agents) {
        ...
        if (agentModes) {
          mergedPermissions[agent] = agentModes
        } else {
          reject(err)
        }
      }
