 ...
 * @param accessLevel {String} Type registry access level
 *   (whether to unregister from a listed or unlisted index).
 *   One of 'listed' or 'unlisted'.
 * @param [location] {String} If present, only unregister the class from this
 *   location (absolute URI).
 * @return {Promise<SolidProfile>}
 */
function removeFromTypeIndex (profile, rdfClass, accessLevel, location) {
  var registryUri
  var registryGraph
  if (accessLevel === 'listed') {
    registryUri = profile.typeIndexListed.uri
 ...
 * @return {Promise<SolidProfile>}
 */
function unregisterType (profile, rdfClass, accessLevel, location) {
  if (!profile) {
    throw new Error('No profile provided')
  }
  if (!profile.isLoaded) {
  ...
  return loadTypeRegistry(profile)  // make sure type registry is loaded
    .then(function (profile) {
      ...
      return removeFromTypeIndex(profile, rdfClass, accessLevel, location)
    })
}
