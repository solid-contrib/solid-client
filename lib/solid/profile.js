...
function SolidProfile (profileUrl, parsedProfile, response) {
  ...
  this.storage = []
  /**
   * Public (Listed) Type registry index (link and parsed graph)
  ...
  /**
   * Private (Unlisted) Type registry index (link and parsed graph)
...
SolidProfile.prototype.appendFromGraph =
  function appendFromGraph (parsedProfile, profileUrl) {
    ...
    if (!this.typeIndexPublic.uri) {
      this.typeIndexPublic.uri = parseLink(parsedProfile, webId,
        vocab.solid('publicTypeIndex'))
    }
    if (!this.typeIndexPrivate.uri) {
      this.typeIndexPrivate.uri = parseLink(parsedProfile, webId,
