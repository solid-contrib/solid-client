 ...
 */
function addToAppRegistry (profile, app) {
 ...
 */
function appsForType (profile, type) {
  var registrations = []
  var isListed = true
  return registrations
    .concat(
      // Public/listed registrations
      registrationsFromGraph(profile.appRegistryListed.graph, type, isListed)
    )
    .concat(
      // Private/unlisted registrations
      registrationsFromGraph(profile.appRegistryUnlisted.graph, type, !isListed)
    )
}
 ...
 * @return {Promise<SolidProfile>} Returns updated profile.
 */
function registerApp (profile, app) {
  if (!profile) {
  ...
  return loadAppRegistry(profile)  // make sure app registry is loaded
    ...
    .then(function (profile) {
      ...
      return addToAppRegistry(profile, app)
    })
}
 ...
 * @return {Array<AppRegistration>}
 */
function registrationsFromGraph (graph, type, isListed) {
  var entrySubject
  var registrations = []
  if (!graph) {
    return registrations
  }
  graph.statementsMatching(null, vocab.app('commonType'), type)
    .forEach(function (entry) {
      entrySubject = entry.subject
      var app = new AppRegistration()
      app.initFromGraph(entrySubject, graph)
